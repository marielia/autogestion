...
<f:view>
	...
	<%/*	
		<c:if var="puedeIngresarS" test="${!subirArchivoBean.puedeIngresar}">		
	...
	<h:column>		  			
		...
		<t:panelGrid border="0" cellpadding="0" cellspacing="0" 
			...
			<t:column>
				<t:outputText value="#{Message.tit_procesa_archivo_autorizaciones}"/>
			</t:column>
			<t:column><h:outputText  value="&nbsp;&nbsp;" escape="false" /></t:column>
		</t:panelGrid>	
		...
		<%/*FILTROS*/%>		
			<h:panelGrid columns="4" width="95%" columnClasses="cpo8,cpo8,cpo8,cpo8" styleClass="columnaTablaCentrada" cellspacing="0" cellpadding="4">						 
				<h:column>
					<t:outputLabel for="consulte" styleClass="titulos">
						<h:outputText  value="&nbsp;#{Message.datos_de_proceso_autorizaciones} " escape="false" styleClass="titulos"/>
					</t:outputLabel>
				</h:column>	
				<h:column></h:column>
				<h:column></h:column>
			...
			<h:panelGrid columns="2" width="95%" columnClasses="cpo8,cpo8" styleClass="columnaTablaCentrada" cellspacing="0" cellpadding="4">						 
			
				<%/*  vendedor  */%>
				<t:column>	
						<t:outputText id="cboVenLbl"  styleClass="cpo8" value="&nbsp;&nbsp;&nbsp;#{Message.vendedor_ccs_label}&nbsp;&nbsp;" escape="false" />							
						<t:outputText styleClass="cpo8" id="apeVenLbl" value="&nbsp;&nbsp;&nbsp;#{Message.apellido_label}&nbsp;&nbsp;" escape="false" />							
				</t:column>
				<t:column >
					
					
					<t:selectOneListbox id="cboVen"  size="1" value="#{subirArchivoBean.vendedor}" styleClass="campo" immediate="true" >					
						<f:selectItems value="#{subirArchivoBean.vendedores}" />
					</t:selectOneListbox>
					         			          				
					<t:inputText id="apeVen" value="#{subirArchivoBean.apellidoVendedor}" styleClass="campo" maxlength="60" size="20"/>				
					...
					<t:inputHidden id="nuevoVendedor" value="#{subirArchivoBean.nuevoVendedor}" />
				...
				<t:column>					
							<t:outputText  styleClass="cpo8" id="nomVenLbl" value="&nbsp;&nbsp;&nbsp;#{Message.nombre_label}&nbsp;&nbsp;" escape="false" />							
				</t:column>
				
				<t:column>					
						<t:inputText id="nombre" value="#{subirArchivoBean.nombreVendedor}" styleClass="campo" maxlength="60" size="20"/>				
				...
				<t:column>
					<h:selectOneListbox id="ccss" size="1" value="#{subirArchivoBean.ccss}" styleClass="campo" 
						  immediate="true"   >						 
						<f:selectItems value="#{subirArchivoBean.lstccss}" />
					</h:selectOneListbox>
					<h:outputText value="&nbsp;[x]" styleClass="datoObligatorio" escape="false"/>	
				</t:column>	
				...
				<t:column>					
					<t:inputFileUpload size="70" id="fichero" value ="#{subirArchivoBean.url}"  storage="file" styleClass="campo" /> 				
				</t:column>	
			</h:panelGrid>
			...
			<h:panelGrid columns="1" width="95%" columnClasses="cpo8Minuscula" styleClass="columnaTablaCentrada" cellspacing="0" cellpadding="4">						 
				<t:column>	
					...
					<h:outputLink  rendered="#{subirArchivoBean.rutaArchivoABaja!=''}" target="_blank" styleClass="cpo7b" value="#{subirArchivoBean.rutaArchivoABaja}">
			...
			<h:panelGrid columns="1" width="95%" columnClasses="cpo8" styleClass="columnaTablaCentrada" cellspacing="0" cellpadding="4">						 							
				<h:column>
					<h:panelGrid columns="1" width="100%"  columnClasses="fondo" styleClass="columnaTablaCentrada" cellspacing="0">						 
				     	<h:column>					        	
				    		<t:commandButton action="#{subirArchivoBean.enviar}"
		    ...
		    <t:saveState value="#{subirArchivoBean.vendedor}"></t:saveState>
			<t:saveState value="#{subirArchivoBean.vendedores}"></t:saveState>	
			<t:saveState value="#{subirArchivoBean.ccss}"></t:saveState>
			<t:saveState value="#{subirArchivoBean.lstccss}"></t:saveState>
...
<html>
	...
	<t:inputFileUpload  id="fichero" value ="#{subirArchivoBean.url}" styleClass="botton"  
	storage="file"  /> 

	<h:commandButton action="#{subirArchivoBean.enviar}"
				value="#{labels.send}" />
  ...
  <t:outputText value="File Uploaded Successfully." 
		rendered="#{subirArchivoBean.rendSuccess}" 
		style="color:green;font-weight:bold"/>
    <t:outputText value="Error in File Uploading." 
		rendered="#{subirArchivoBean.rendFailure}" 
