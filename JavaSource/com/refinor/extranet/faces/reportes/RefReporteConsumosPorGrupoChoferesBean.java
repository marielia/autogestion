...
public class RefReporteConsumosPorGrupoChoferesBean extends AbstListado {

	public RefReporteConsumosPorGrupoChoferesBean() {
	...
	public void buscar(ActionEvent event)throws NoExistenItemsException, DataAccessErrorException ,Exception{
		try{			
			...
			if(getTipoUsuarioLogueado()==1){
				//es cliente
				setItems(mPedidoDAO.getConsumosPorChofer(codCliente,fltFechaDesde,fltFechaHasta, fltNombreChofer,getUnidadNegocio(), fltDescripcionUnidadNegocio,getGrupoUnidadNegocio(), fltDescripcionGrupoUN,true));	
				...
				setItems(mPedidoDAO.getConsumosPorChofer(codClienteAlfa,fltFechaDesde,fltFechaHasta, fltNombreChofer,getUnidadNegocio(),fltDescripcionUnidadNegocio,getGrupoUnidadNegocio(), fltDescripcionGrupoUN,true));	
	...
	public void generarExcel() {
		try{			
			System.out.println("Paso por generar excel de consumo por grupo chofer !!");
			if(getItems()!=null && getItems().size()!=0){
				ExportarExcelBean exportarExcelBean =  new ExportarExcelBean();			
				this.nombreArchivo = exportarExcelBean.generarExcelReporteConsumosPorGrupo(getItems(),getFechaActualStr(),false,getTipoUsuarioLogueado());
			}
	...
	public void verRemitos(ActionEvent event){
		try{
			...
			setItems2(mPedidosDAO.getRemitosConsumoChoferesPorClientePatenteProductoFechas(nroCliente,dni,nroProducto,getFltFechaDesde(),getFltFechaHasta(),nroUnidNeg,nroGrupoUnidNeg,true));
